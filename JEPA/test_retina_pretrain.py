                        ...
                        def _pca_after():
                            ...
                            if feats:
                                ...
                                try:
                                    ...
                                    pass
                                cont_plot([eval_maps['age_map'].get(rc, None) for rc in regs], 'Age', 'pca_after_age.png')
                                cont_plot([eval_maps['bmi_map'].get(rc, None) for rc in regs], 'BMI', 'pca_after_bmi.png')
                        _pca_after()
...
def main():
    ...
    if demo_maps is not None and 'RegistrationCode' in man_df.columns:
        ...
        eval_loader = DataLoader(EvalDataset(eval_df, trsf), batch_size=CONFIG['batch_size'], shuffle=False, num_workers=0)
    else:
        eval_loader = None
    ...
    print(f"Data pipeline: batch_size={CONFIG['batch_size']}, img_size={CONFIG['img_size']}, normalization=mean/std {CONFIG.get('mean', '[default]')}/{CONFIG.get('std', '[default]')}")
    ...
    if stage is None or stage == 'pretrain':
        ...
        try:
            if eval_loader is not None and demo_maps is not None and 'sex_map' in demo_maps:
                regs = set(eval_loader.dataset.df.get('RegistrationCode', pd.Series([], dtype=str)).astype(str))
        ...
        run_masked_pretrain(enc, pred, loader_train, loader_val, CONFIG, start_epoch=epoch, elapsed_time=elapsed_time, demo_eval=(eval_loader, demo_maps, metrics_path))
